...
function General({ onShift, setColname, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:4000/api/colgendetails', formData)
  ...
  useEffect(() => {
    const fetchmedidata = async () => {
      try {
        const response = await fetch("http://localhost:4000/api/getallmedidata")
        if (response.ok) {
          ...
          const collaff = AllData.filter(item => item.category === 'Affiliated' && item.type === 'College')
          setColaf(collaff)

          const colap = AllData.filter(item => item.category === 'Approaved' && item.type === 'College')
          setcolap(colap)
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-7">
          ...
          <div>
            ...
            <div className="input-area relative flex">
              <div className="flex space-x-4 items-center mb-3">
                <input type="radio" id="Private College" name="collegeType" value="Private College" onChange={(e) => setColtype(e.target.value)} required />
              ...
              <div className="flex space-x-4 items-center mb-3 ml-5">
                <input type="radio" id="Government College" name="collegeType" value="Government College" onChange={(e) => setColtype(e.target.value)} required />
          ...
          <div className="input-area relative">
            ...
            <input type="text" className="form-control" placeholder="College Full Name" value={colfull} onChange={(e) => setColFull(e.target.value)} required />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
          ...
          <div className="input-area">
            ...
            <Select options={optionsaff} closeMenuOnSelect={true} placeholder="Select options..." value={options.find((selectedOption) => selectedOption.value === affby)} onChange={(selectedOption) => setAffby(selectedOption ? selectedOption.value : null)} />
          </div>

          <div className="input-area">
            <label htmlFor="select" className="form-label">Approaved By<span style={{ color: 'red' }}>*</span></label>
            <Select options={optionsapp} closeMenuOnSelect={true} placeholder="Select options..." value={options.find((selectedOption) => selectedOption.value === appby)} onChange={(selectedOption) => setAppby(selectedOption ? selectedOption.value : null)} />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-7">
